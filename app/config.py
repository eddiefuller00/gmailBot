...
class Settings:
    ...
    database_path: str = _as_str(os.getenv("DATABASE_PATH"), "data/inbox_intelligence.db")
    openai_api_key: str | None = _as_optional_str(os.getenv("OPENAI_API_KEY"))
    openai_chat_model: str = _as_str(os.getenv("OPENAI_CHAT_MODEL"), "gpt-5.4-mini")
    ...
    openai_embedding_model: str = _as_str(
        os.getenv("OPENAI_EMBEDDING_MODEL"), "text-embedding-3-small"
    )
    top_important_default: int = int(os.getenv("TOP_IMPORTANT_DEFAULT", "5"))
    frontend_origins: tuple[str, ...] = _frontend_origins()
    frontend_app_url: str = _as_str(os.getenv("FRONTEND_APP_URL"), frontend_origins[0])

    google_client_id: str | None = _as_optional_str(os.getenv("GOOGLE_CLIENT_ID"))
    google_client_secret: str | None = _as_optional_str(os.getenv("GOOGLE_CLIENT_SECRET"))
    google_redirect_uri: str | None = _as_optional_str(os.getenv("GOOGLE_REDIRECT_URI"))
    google_auth_url: str = _as_str(
        os.getenv("GOOGLE_AUTH_URL"), "https://accounts.google.com/o/oauth2/v2/auth"
    )
    google_token_url: str = _as_str(
    ...
    token_encryption_key: str | None = _as_optional_str(os.getenv("TOKEN_ENCRYPTION_KEY"))
